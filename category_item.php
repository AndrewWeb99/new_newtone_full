...
if (isset($_GET['id'])) {
  $s = $_GET['id'];
  $product = getProduct("WHERE category_id = " . $s); 
}

if (isset($_GET['search'])) {
  $s = $_GET['search'];
  if (isset($_GET['voz_price'])) {
    $product = getProduct("WHERE category_id = $s ORDER BY price ASC");
  }
  if (isset($_GET['ub_price'])) {
    $product = getProduct("WHERE category_id = $s ORDER BY price DESC");
  }
  if (isset($_GET['new'])) {
    $product = getProduct("WHERE category_id = $s ORDER BY id DESC");
  }
  if (isset($_GET['nazv'])) {
    $product = getProduct("WHERE category_id = $s ORDER BY title");
  }
}
?>
<!DOCTYPE html>
      ...
      <div class="container">
        <div class="select">
          <select onchange="window.location.href=this.options[this.selectedIndex].value">
            ...
            <option value="/category_item.php?search=<?= $s; ?>&voz_price=1">по возрастанию цены</option>
            <option value="/category_item.php?search=<?= $s; ?>&ub_price=1">по убыванию цены</option>
            <option value="/category_item.php?search=<?= $s; ?>&new=1">новые</option>
            <option value="/category_item.php?search=<?= $s; ?>&nazv=1">по названию</option>
          </select>
        </div>
        ...
        <div class="cat_box_item">
          ...
          <?
            ...
            echo '<div style="width: 100%; text-align:center; font-size: 25px; color: black;">Товар данной категории отсутствуют</div>';
          }
          ?>
